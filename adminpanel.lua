-- TripleS GUI (standalone final, Fly normal + safer Fling, full script) -- Paste sekali: GUI + semua fungsi sudah ada di sini
...
-- Perbaikan utama:
-- 1) Arah gerak diambil dari ControlModule (analog/WASD) yang diproyeksikan ke kamera -> konsisten.
-- 2) Orientasi mengikuti kamera; kecepatan stabil pakai flySpeed.
-- 3) Tetap bukan noclip; hanya PlatformStand untuk stabilkan fisika saat terbang (tabrakan tetap ada).
local function startFly() 
    ...
    -- Stabilkan humanoid tanpa noclip
    pcall(function()
    ...
    -- akses ControlModule untuk baca vektor gerak (analog/WASD)
    ...
    flyConn = RS.RenderStepped:Connect(function()
        ...
        -- vector input: utamakan ControlModule (lebih konsisten untuk analog), fallback ke MoveDirection
        local move = Vector3.zero
        if controlModule and controlModule.GetMoveVector then
            pcall(function() move = controlModule:GetMoveVector() end)
        end
        if move.Magnitude <= 0 then
            move = hum2.MoveDirection or Vector3.zero
        end

        -- proyeksikan ke arah kamera (tanpa komponen Y)
...
-- Perbaikan utama:
-- - Bukan hanya set AssemblyLinearVelocity sekali.
-- - Memberi "impulse" yang disustain sebentar via BodyVelocity pada target (lalu dibersihkan),
--   sehingga efek fling terlihat jelas, tetap aman buat kita.
local function startFling() 
    ...
    flingConn = RS.Heartbeat:Connect(function(dt)
                ...
                if theirHRP then
                    ...
                    if dist <= 10 then
                        ...
                        if t - last >= flingCooldown then
                            ...
                            -- sustain impulse: BodyVelocity sementara (0.12s)
                            pcall(function()
                                local bv = Instance.new("BodyVelocity")
                                bv.Name = "TripleS_FlingPulse"
                                bv.MaxForce = Vector3.new(9e9,9e9,9e9)
                                -- Away + Upward push (lebih terlihat)
                            ...
                            -- optional: bangunkan humanoid target
                            pcall(function()
...
local function setCheckpoint() 
    ...
    respawnConn = lp.CharacterAdded:Connect(function(char) 
        ...
        if checkpointCF and hrp2 then 
            task.wait(0.25) 
            pcall(function() if hum2 then hum2:ChangeState(Enum.HumanoidStateType.Physics) end end) 
            char:PivotTo(checkpointCF) 
            task.wait(0.05) 
            pcall(function() if hum2 then hum2:ChangeState(Enum.HumanoidStateType.GettingUp) end end) 
        end 
    end) 
end
