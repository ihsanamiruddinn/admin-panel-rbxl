-- TripleS GUI (standalone, no IY) — fly/unfly, fling/unfling, spawn/nospawn, speed 23, tptool, tgoto, gotopart
...
local function safeTeleport(cf)
    ...
    if hrp then
        ...
        if hum then hum:ChangeState(Enum.HumanoidStateType.Physics) task.wait() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end
    end
end
...
local function startFly()
    ...
    local function updateMove()
        ...
        lookCF = CFrame.new(Vector3.zero, (cam.CFrame.LookVector*Vector3.new(1,0,1)).Unit + Vector3.new(0,0.0001,0))
        local dir = moveDir
        local v = (lookCF:VectorToWorldSpace(Vector3.new(dir.X,0,dir.Z)))
        local y = ascend - descend
        flyBV.Velocity = (v * flySpeed) + Vector3.new(0, y*flySpeed, 0)
        flyBG.CFrame = CFrame.new(hrp.Position, hrp.Position + cam.CFrame.LookVector)
    end

    if flyConn then flyConn:Disconnect() end
    flyConn = RS.RenderStepped:Connect(updateMove)
end
...
-- input WASD + Space/Shift buat fly
UIS.InputBegan:Connect(function(i, gpe)
    if gpe then return end
    if not flying then return end
    if i.KeyCode == Enum.KeyCode.W then moveDir = Vector3.new(0,0,-1)
    elseif i.KeyCode == Enum.KeyCode.S then moveDir = Vector3.new(0,0,1)
    elseif i.KeyCode == Enum.KeyCode.A then moveDir = Vector3.new(-1,0,0)
    elseif i.KeyCode == Enum.KeyCode.D then moveDir = Vector3.new(1,0,0)
    elseif i.KeyCode == Enum.KeyCode.Space then ascend = 1
    elseif i.KeyCode == Enum.KeyCode.LeftShift or i.KeyCode == Enum.KeyCode.RightShift then descend = 1
    end
end)
...
local function startFling()
    ...
    flingConn = RS.Heartbeat:Connect(function()
        ...
        -- putar kuat biar fisik “nyenggol” pemain lain
        h.AssemblyAngularVelocity = Vector3.new(0, 1500, 0)
        -- sedikit dorong
        h.AssemblyLinearVelocity += (h.CFrame.LookVector * 5)
    end)
end
...
local function stopFling()
    ...
    flingActive = false
    if flingConn then flingConn:Disconnect() flingConn=nil end
    local _, hrp = getChar(); if hrp then
        hrp.AssemblyAngularVelocity = Vector3.zero
    end
end
...
local function setSpeed(v)
    local _, _, hum = getChar(); if hum then hum.WalkSpeed = v end
end
...
local function gotoPartByName(partName)
    ...
    safeTeleport(target.CFrame + Vector3.new(0,3,0))
end

-- ===== Command Router (dipakai tombol & textbox) =====
local function runCommand(cmd)
    ...
    elseif h == "fling" then
        startFling()
    elseif h == "unfling" then
        stopFling()
    elseif h == "spawn" then
        setCheckpoint()
    elseif h == "nospawn" then
        clearCheckpoint()
    elseif h == "speed" then
        local v = tonumber(args[2] or "")
        if v then setSpeed(v) end
    elseif h == "tptool" then
        ensureTpTool()
    elseif h == "tgoto" then
        gotoPlayer(args[2] and table.concat(args, " ", 2) or "")
    elseif h == "gotopart" then
        gotoPartByName(args[2] and table.concat(args, " ", 2) or "")
    end
end
...
-- input bar (tetap ada, opsional kamu pakai sendiri)
...
input.FocusLost:Connect(function(enter) if enter and input.Text~="" then runCommand(input.Text) input.Text="" end end)

-- content (tombol utama) — (tetap Frame sesuai permintaan)
...
local function makeInput(parent, placeholder, baseCommand)
    ...
    box.PlaceholderText = placeholder   -- << tetap "TGoto" / "GotoPart"
    box.Text = ""
...
-- ganti jadi input box (sesuai instruksi)
